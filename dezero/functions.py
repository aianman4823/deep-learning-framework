...
def sigmoid_simple(x):
    x = as_variable(x)
    sigmoid_range = as_variable(np.array(34.538776394910684))

    if x.data <= -sigmoid_range.data:
        return as_variable(1e-15)
    if x.data >= sigmoid_range.data:
        return as_variable(1.0 - 1e-15)
...
def softmax_simple(x, axis=1):
    x = as_variable(x)
    y = exp(x)
    # keepdims=Trueとすることで行ごとの割り算が行われる
